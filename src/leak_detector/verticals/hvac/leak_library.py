"""
Leak Detector v1 — HVAC Leak Library
...
HVAC_LEAK_LIBRARY={
"HVAC-LEAK-001":L("HVAC-LEAK-001","Missed calls and slow lead response",LeakCategory.LEAD_CAPTURE,"Inbound calls go unanswered or are never followed up.",[S(DetectionSignal.MISSED_CALL_RATE_HIGH,.40),S(DetectionSignal.NO_AFTER_HOURS_CAPTURE,.30),S(DetectionSignal.NO_SMS_TEXTBACK,.30)],["monthly_inbound_calls","missed_call_rate","after_hours_call_share","call_to_book_rate","average_service_ticket","contribution_margin_per_job"],2,7,"Deploy missed-call text-back + AI receptionist / after-hours routing",True,"HVAC-MISSED-CALL-V1","leak_detector.verticals.hvac.playbooks.missed_call","Operational fix required (§17.2)."),
"HVAC-LEAK-002":L("HVAC-LEAK-002","Booking friction on website and mobile",LeakCategory.CONVERSION_FRICTION,"Visitors face friction booking or requesting service.",[S(DetectionSignal.NO_ONLINE_BOOKING,.35),S(DetectionSignal.NO_CTA_ABOVE_FOLD,.25),S(DetectionSignal.NO_CLICK_TO_CALL,.20),S(DetectionSignal.FORM_TOO_LONG,.20),S(DetectionSignal.MOBILE_BOOKING_BROKEN,.25)],["monthly_inbound_calls","web_form_to_conversation_rate","average_service_ticket","contribution_margin_per_job"],2,10,"Simplify booking flow: above-fold CTA, short form, one-tap call, and an online booking path"),
...
"HVAC-LEAK-011":L("HVAC-LEAK-011","No financing options visible",LeakCategory.CONVERSION_FRICTION,"No financing or payment-plan option shown on the public site, which costs the highest-ticket replacement jobs to competitors who offer one.",[S(DetectionSignal.NO_FINANCING_OPTIONS,1.0)],["average_replacement_ticket","contribution_margin_per_job"],2,14,"Add a financing partner (e.g. Synchrony, Wisetack, Service Finance) and surface it on the homepage and service pages"),
"HVAC-LEAK-012":L("HVAC-LEAK-012","No instant-contact channel",LeakCategory.LEAD_CAPTURE,"No chat widget or SMS option for visitors who won't pick up the phone to make first contact.",[S(DetectionSignal.NO_INSTANT_CONTACT_CHANNEL,1.0)],["monthly_inbound_calls","average_service_ticket","contribution_margin_per_job"],2,10,"Add a chat widget or SMS-first contact option alongside the phone number")}
